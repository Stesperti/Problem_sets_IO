...
function msm_moments(params, test_scores, sports, distance, y, R)
    ...
    for i in 1:N
        for j in 1:J
            z_ij = [test_scores[j], sports[j], distance[i,j]]  # vector of instruments
            for z in z_ij
                push!(g, (Int(y[i] == j) - P[i,j]) * z)
            end
        end
    end
...
function msm_objective(params, test_scores, sports, distance, y, R)
    g = msm_moments(params, test_scores, sports, distance, y, R)
    return dot(g, g)  # simple identity weighting
end
...
function msm_objective_efficient(params, test_scores, sports, distance, y, R)
    g_all = msm_moments(params, test_scores, sports, distance, y, R) 
    # g_all: N x L, each row = g_i
    N = size(g_all, 1)
    g_mean = mean(g_all, dims=1)
    Omega = (g_all .- g_mean)' * (g_all .- g_mean) / N  # L x L covariance
    W = inv(Omega)
    g = vec(sum(g_all, dims=1))  # sum moments across individuals (stack)
    return dot(g, W * g)
end
